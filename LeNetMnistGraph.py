...
from logisticMnistGraph import LogisticRegression, load_data
...
def evaluate_lenet5(learning_rate=0.05, n_epochs=1,
                    dataset='mnist.pkl.gz',
                    nkerns=[20, 50], batch_size=2): # epoch no was 200 # batch size was 500
    ...
    patience = 20 # was 10000  # look as this many examples regardless
    patience_increase = 2  # wait this much longer when a new best is
                           # found
    improvement_threshold = 0.9995 #was 0.995 # a relative improvement of this much is
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(int(n_train_batches)):
            ...
            if iter>2:
                ...
                print('starting')
                print(filterHolder[int(iter-1)][0:5,0:5]-filterHolder[int(iter)-2][0:5,0:5])
                print(filterHolder[int(iter-1)][0:5,5:10]-filterHolder[int(iter)-2][0:5,5:10])
                print(filterHolder[int(iter-1)][0:5,0:5].shape)
                print('end 1 and 2')
                print(filterHolder[int(iter-1)][2].shape)
                print(filterHolder[int(iter-1)][3].shape)
                print(filterHolder[int(iter-1)][0:5,10:15]-filterHolder[int(iter)-2][0:5,10:15])
                print(filterHolder[int(iter-1)][0:5,15:20]-filterHolder[int(iter)-2][0:5,15:20])
            #print('shapes')
            #print(hiddenW.shape)
            #print(logRWT.shape)
            ...
            if (iter + 1) % validation_frequency == 0:
                print('iter: '+str(iter))
                print('valid freq: '+ str(validation_frequency))
